	...
	class Payments extends CI_Controller
	{
		public function dashboard(){
			if(!$this->session->userdata('login')) {
				redirect('payments/login');
			}
			$data['title'] = 'Dashboard';

			$this->load->view('templates/header');
			$this->load->view('payments/dashboard', $data);
		...
		public function register(){
			...
			if($this->form_validation->run() === FALSE){
				...
				$this->Payment_Model->register($encrypt_password);
		...
		public function login(){
			...
			if($this->form_validation->run() === FALSE){
				$this->load->view('templates/header');
				$this->load->view('payments/login', $data);
				$this->load->view('templates/footer');
			}else{
				...
				$user_id = $this->Payment_Model->login($username, $encrypt_password);
		...
		public function check_username_exists($username){
			...
			if ($this->Payment_Model->check_username_exists($username)) {
				return true;
			}else{
				return false;
			}
		...
		public function check_email_exists($email){
			...
			if ($this->Payment_Model->check_email_exists($email)) {
				return true;
			}else{
				return false;
			}
