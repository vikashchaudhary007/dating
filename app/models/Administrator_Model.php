<?php
	class Administrator_Model extends CI_Model
	{
		...
		public function get_users_payment($username = FALSE, $limit = FALSE, $offset = FALSE){
			if ($limit) {
				$this->db->limit($limit, $offset);
			}
			if($username === FALSE){
				$this->db->order_by('payments.payment_id', 'DESC');
				$this->db->join('user_registration','user_registration.id =payments.item_number');
				$query =$this->db->get('payments');
				return $query->result_array(); 
			}
			$this->db->from('payments');
			$this->db->join('user_registration','user_registration.id =payments.item_number');
			$query = $this->db->where('payments.username',$username);
			return $query->row_array(); 
